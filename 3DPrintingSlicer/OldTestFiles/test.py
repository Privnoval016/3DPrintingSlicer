...
class ModernGCodeVisualizerGUI(QMainWindow):
    ...
    def setupControlPanel(self, parent):
        ...
        # Add stretch to push content to top but allow scrolling
        layout.addStretch()

        # Set minimum height for the content to ensure scrolling works
    ...
    def setupLayerSettingsGroup(self, parent_layout):
        ...
        self.value_input.returnPressed.connect(self.apply_settings)
    ...
    def load_from_gcode(self):
        """Load and setup G-code visualization"""
    ...
    def compute_axis_limits(self):
        """Compute axis limits from slice data (similar to your original _compute_axis_limits)"""
        if not self.slices:
            return
    ...
    def toggle_auto_bounds(self, checked):
        """Enable/disable manual bounds input"""
    ...
    def set_custom_bounds(self, x_min=0, x_max=100, y_min=0, y_max=100, z_min=0,
                          z_max=100):
        """Programmatically set custom bounds"""
        self.auto_bounds_checkbox.setChecked(False)
        self.toggle_auto_bounds(False)
    ...
    def update_slices(self, index):
        ...
        for i in indices:
            ...
            if len(slice_data.vertices) == 0 or len(slice_data.edges) == 0:
                continue

            lines = []
            for edge in slice_data.edges:
                if edge[0] < len(slice_data.vertices) and edge[1] < len(
                        slice_data.vertices):
                    lines.append([slice_data.vertices[edge[0]],
                                  slice_data.vertices[edge[1]]])
    ...
    def update_operation_lines(self, index):
        """Update operation lines for G-code visualization - matches original behavior"""
        ...
        if index < self.gcode_evaluator.index:
            self.gcode_evaluator.index = 0
            if hasattr(self.gcode_evaluator, 'actual_position'):
                self.gcode_evaluator.actual_position = [0, 0,
                                                        0]  # Reset to origin
        ...
        while self.gcode_evaluator.index <= index:
            ...
            if (should_draw and not np.array_equal(start_pos, end_pos) and
                ...
                # Create line collection for this operation
                lc = Line3DCollection([line], colors='red',
                                      linewidths=line_width, alpha=alpha)
                self.ax.add_collection3d(lc)
                self.visible_operation_lines[current_op_index] = lc
    ...
    def start_autoplay(self):
        self.autoplay_timer.start(500)  # 500ms between steps
...
def main():
    # This would need to be imported from your existing modules
    # from your_module import ZSlicer, GCodeEvaluator

    app = QApplication(sys.argv)
    app.setStyle('Fusion')  # Modern looking style

    # Initialize your existing classes here
    # z_slicer = ZSlicer()
    # gcode_evaluator = GCodeEvaluator()

    # For demonstration, we'll use None - replace with actual instances
    window = ModernGCodeVisualizerGUI(None, None)
    window.show()
